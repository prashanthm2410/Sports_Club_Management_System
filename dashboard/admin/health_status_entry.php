...
if (isset($_POST['submit'])) {
    ...
    // Check if the entry with the given uid exists
    $checkQuery = "SELECT * FROM health_status WHERE uid = '$userid'";
    $checkResult = mysqli_query($con, $checkQuery);

    if (mysqli_num_rows($checkResult) > 0) {
        // If entry exists, perform an update
        $query = "UPDATE health_status 
                  SET calorie='$calorie', height='$height', weight='$weight', fat='$fat', remarks='$remarks' 
                  WHERE uid='$userid'";
    } else {
        // If entry doesn't exist, perform an insert
        $query = "INSERT INTO health_status (calorie, height, weight, fat, remarks, uid) 
                  VALUES ('$calorie', '$height', '$weight', '$fat', '$remarks', '$userid')";
    }
    ...
    if ($result) {
        ...
        $cal = isset($row['calorie']) ? $row['calorie'] : null;
        $hei = isset($row['height']) ? $row['height'] : null;
        $wei = isset($row['weight']) ? $row['weight'] : null;
        $fa = isset($row['fat']) ? $row['fat'] : null;
        $remar = isset($row['remarks']) ? $row['remarks'] : null;
    }
}
?>
<!DOCTYPE html>
                        ...
                        <td height="35">
                           <table width="100%" border="0" align="center">
                              ...
                              <tr>
                                 ...
                                 <td height="35"><input type="text" id="boxx" name="height" value='<?php echo $hei?>'placeholder="Enter Height in foot"></td>
                              </tr>
                              <tr>
